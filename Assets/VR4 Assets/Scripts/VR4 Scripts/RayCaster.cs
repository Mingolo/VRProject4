...
public class RayCaster : MonoBehaviour
{
    ...
    public LayerMask objectLayer;                                       //set in inspector
    ...
    void RayCasting()
    {
        ...
        if ( Physics.Raycast(ray, out hit, lookDist, objectLayer))                                //use SphereCast instead
        {
    ...
    void ObjectIDAndSelect(GameObject go)
    {
        ...
        else if (go == lastObjectHit)
        {
            ...
            {
                // print("Charging");
                go.GetComponentInChildren<ItemSelection>().isOver = true;
            }
        ...
        else if (lastObjectHit != go)
        {
            ...
            if (lastObjectHit != null && (lastObjectHit.tag == "Phone" || lastObjectHit.tag == "Beer" || lastObjectHit.tag == "Water" || lastObjectHit.tag == "Keys"))          //CHANGE THIS TO WHATEVER TAGS WE ARE USING FOR THE OBJECTS
                lastObjectHit.GetComponentInChildren<ItemSelection>().isOver = false;
    ...
    void ObjectCheck(GameObject go)
    {
        ...
        if (go != null &&
            isOff &&
            lastObjectHit.GetComponent<ItemSelection>() != null &&
            go.GetComponent<ItemSelection>() == null)
        {
            lastObjectHit.GetComponent<ItemSelection>().isOver = false;
        }
        isOff = false;
